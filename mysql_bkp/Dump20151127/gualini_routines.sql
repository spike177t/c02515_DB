...
-- 		, IF(cass_riferimento - ID_CASSETTO >= 0, cass_riferimento - ID_CASSETTO, MAX_CASS_MAG1 + (cass_riferimento - ID_CASSETTO)) AS DIST_CCW
	...
	DIST_CLW
	LIMIT 1
    INTO DISTANZA_MAG1_CLW
	;
    
     -- assegno DISTANZA_MAG1_CCW   
        SELECT 
-- 		, IF(ID_CASSETTO - cass_riferimento >= 0, ID_CASSETTO - cass_riferimento, MAX_CASS_MAG1 + (ID_CASSETTO - cass_riferimento)) AS DIST_CLW 
-- 		, 
		IF(cass_riferimento - ID_CASSETTO >= 0, cass_riferimento - ID_CASSETTO, MAX_CASS_MAG1 + (cass_riferimento - ID_CASSETTO)) AS DIST_CCW
	...
	DIST_CLW
	LIMIT 1
    INTO  DISTANZA_MAG1_CCW
	;
    
    -- assegno DISTANZA_MAG2_CLW
    SELECT
		IF(ID_CASSETTO - cass_riferimento >= 0, ID_CASSETTO - cass_riferimento, MAX_CASS_MAG1 + (ID_CASSETTO - cass_riferimento)) AS DIST_CLW 
-- 		, IF(cass_riferimento - ID_CASSETTO >= 0, cass_riferimento - ID_CASSETTO, MAX_CASS_MAG1 + (cass_riferimento - ID_CASSETTO)) AS DIST_CCW
	FROM gualini.cassetti_disponibili 
	...
	DIST_CLW
	LIMIT 1
    INTO DISTANZA_MAG2_CLW
	;
    -- assegno DISTANZA_MAG2_CCW
    SELECT 
-- 		IF(ID_CASSETTO - cass_riferimento >= 0, ID_CASSETTO - cass_riferimento, MAX_CASS_MAG1 + (ID_CASSETTO - cass_riferimento)) AS DIST_CLW 
 		IF(cass_riferimento - ID_CASSETTO >= 0, cass_riferimento - ID_CASSETTO, MAX_CASS_MAG1 + (cass_riferimento - ID_CASSETTO)) AS DIST_CCW
	FROM gualini.cassetti_disponibili 
...
ricerca: REPEAT
        ...
        IF cass_riferimento = NULL THEN        
			SELECT ID_CASSETTO from cassetti_occupati WHERE  magazzino_ID_MAGAZZINO = valMAGAZZINO Order By lastupdate_milli LIMIT 1;
        END IF;
        SET cass_riferimento = IF(cass_riferimento != NULL, cass_riferimento, 1); -- potrei ottimizzare sapendo qual'è il cassetto di fronte all'estrattore 
...
-- 		, IF(ID_CASSETTO - cass_riferimento >= 0, ID_CASSETTO - cass_riferimento, MAX_CASS_MAG1 + (ID_CASSETTO - cass_riferimento)) AS DIST_CLW 
    ...
    SELECT magazzino AS MAGAZZINO, cassetto AS CASSETTO;
...
-- Dump completed on 2015-11-27  9:05:14
